import { type ActivityOptions, ActivityType, type Client, type ClientUser } from 'discord.js'
...
const random = <T>(stuff: T[]) => stuff[Math.floor(Math.random() * stuff.length)]

const createStatusOption = (status: ClientStatus): Omit<ActivityOptions, 'name'> | undefined => {
...
export const setUsername = (user: ClientUser, username: string) => tryInvoke(() => user.setUsername(username))
export const setStatus = (user: ClientUser, status: ClientStatus) => tryInvoke(() => user.setActivity(status.statusText, createStatusOption(status)))

export const setRandomUsername = (user: ClientUser, usernames: string[]) => tryInvoke(() => user.setUsername(random(usernames)))
...
export const buildActivitySystem = (client: Client) => {
  ...
  return {
    enabled: () => !!timer,
    enableActivitySelector: async (statusCollection: ClientStatus[], usernames: string[], rotationTimeInMs = 45 * 60 * 1000) => {
