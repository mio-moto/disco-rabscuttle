...
export const ohlcMin = (entry: OHLCDataPoint): number =>
  Math.min.apply(null, [entry.open, entry.high, entry.low, entry.close]);

export const ohlcMax = (entry: OHLCDataPoint): number =>
  Math.max.apply(null, [entry.open, entry.high, entry.low, entry.close]);
