...
export const buildVoter = async (db: PromisedDatabase) => {
    await db.createTable(VotingTable, true, "message TEXT NOT NULL", "user TEXT NOT NULL", "weight INTEGER", "PRIMARY KEY ( message, user )");
    await db.createTable(VotingOpeningTable, true, "channel TEXT NOT NULL", "message TEXT NOT NULL PRIMARY KEY", "time TEXT NOT NULL");
    return {
        submitVote: async (messageId: string, user: string, value: number) => {
            await db.replace(VotingTable, { message: messageId, user: user, weight: value });
        ...
        getVotingsOlderThan: async (minutes: number) => {
            return (await db.all(SelectOlderThan, `-${minutes} minute`)) as ({'channel': string, 'message': string}[]) 
        },
