...
export const buildHasher = (
  ...
  return {
    // get static singular
    hashUltra: (message: string) =>
      buildHit(Rarity.ultra, ultraHasher(message)),
    hashRare: (message: string) => buildHit(Rarity.rare, rareHasher(message)),
    hashNormal: (message: string) =>
      buildHit(Rarity.normal, normalHasher(message)),
    ...
    gachaPepe: function (text?: string | null): GachaHit {
      ...
      return buildHit(
        Rarity.normal,
        text ? normalHasher(text) : randomizer.randomNormal()
      );
    },
