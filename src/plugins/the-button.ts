import { ActionRowBuilder, ButtonBuilder, type ButtonInteraction, type ButtonStyle, type Client } from 'discord.js'
import type { Kysely } from 'kysely'
import type { Logger } from 'winston'
import type { ButtonPlugin } from '../message/hooks'
...
const bootstrapDatabase = async (db: Kysely<unknown>): Promise<Kysely<Database>> => {
  await db.schema
    .createTable(ButtonTable)
    .ifNotExists()
...
const postButton = async (channelId: string, client: Client, db: Kysely<Database>, buttonProposal: ButtonEvent) => {
  ...
  const button = new ButtonBuilder().setStyle(buttonProposal.buttonStyle).setLabel(buttonProposal.buttonText).setCustomId('the-evil-button')
...
export const TheButtonPlugin: ButtonPlugin = {
  ...
  onInit: async function (client, db, config, logger) {
    const database = await bootstrapDatabase(db)
    ...
    setInterval(async () => {
      ...
      for (const channel of pluginConfig.buttonPlugin.targetChannels) {
        ...
        buttonLogic(channel, client, database, proposals)
        // spacing this apart a bit, so that it doesn't hammer the API
        await waitFor(250)
      }
      await waitFor(DefaultInterval - jitter)
    }, DefaultInterval)
    this.onNewButtonClick = async (interaction) => {
      try {
        await claimButton(interaction, database, logger)
      } catch (e) {
