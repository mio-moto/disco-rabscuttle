...
const tools = ['JS', 'TypeScript', 'ModDota', 'IRC', 'Discord', 'toilet', 'Panorama', 'Lua']

const reaction = ['watch weeb shit', 'code', 'shut the fuck up', 'rub ${random} nippels', 'listen to ${random}, you little shit', 'chillax']
...
function format(text: string): string {
  ...
  while (entryText !== resultText) {
    entryText = resultText
    if (resultText.includes('${tool}')) {
      resultText = resultText.replace('${tool}', selectRandom(tools))
      continue
    }

    if (resultText.includes('${reaction}')) {
      resultText = resultText.replace('$reaction', selectRandom(reaction))
      continue
    }

    if (resultText.includes('${language}')) {
      const language = selectRandom(languages)
      resultText = resultText.replace('${language}', Math.random() > 0.5 ? language.name : language.nativeName)
...
function setNames(text: string, username: string, random: string, botname: string) {
  return text.split('${username}').join(username).split('${random}').join(random).split('${botname}').join(botname)
}
