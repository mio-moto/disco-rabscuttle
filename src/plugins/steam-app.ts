...
const APP_DETAILS_URL =
  'https://store.steampowered.com/api/appdetails?appids={ID}&cc=it&l=english';
...
const loadAppList = async () => {
  ...
  if (!response.ok) {
    logger.error(
      `Tried to fetch app list, but failed with status: ${response.status}: ${response.statusText}`
    );
...
const loadAppDetails = async (id: number) => {
  ...
  if (!response.ok) {
    logger.error(
      `Tried to fetch app details for id [${id}], but failed with status: ${response.status}: ${response.statusText}`
    );
  ...
  if (!result.success) {
    throw new Error(
      `App Details API call worked, but API replied with no success indicator (obj['${id}'].success == false)`
    );
  }
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise((resolve, reject) =>
        setTimeout(() => resolve(new Error('timeout')), 1500)
      ),
    ]);
    if (!resolved) {
...
const findAppId = (input: string) => {
  ...
  return (
    appList.applist.apps.find(x => x.appid == numeric) ??
    appList.applist.apps.find(x => x.name == input)
  );
};
  ...
  try {
    ...
    if (!embed) {
      return await interactionError(
        interaction,
        "Sorry, couldn't find or translate the app",
        10_000
      );
    }
    const options: InteractionReplyOptions = {embeds: [embed]};
    if (hasMessage) {
      const userPreamble = interaction.member
        ? `${interaction.member.user} said:\n`
        : '';
      options['content'] = `${userPreamble}> ${message}`;
      // first show the error
      const errorPromise = interactionError(
        interaction,
        'Replying again as text message, otherwise mentions will not work, sorry.'
      );
  ...
  descriptor: {
    ...
    options: [
      ...
      },
      {
        ...
        name: 'comment',
        description:
          "Optional comment to give some context why you've linking this",
        required: false,
        autocomplete: false,
      },
    ],
    ...
    const titles = appList.applist.apps
      .filter(
        x =>
          ...
          `${x.appid}`.startsWith(searchString)
      )
      .sort((a, b) => a.appid - b.appid)
      .splice(0, 25);
    return await interaction.respond(
      titles.map(x => ({name: `[${x.appid}] ${x.name}`, value: `${x.appid}`}))
    );
  },
};
