...
const buildEmbed = async (page: Page): Promise<EmbedBuilder> => {
  ...
  embed.setAuthor({
    name: page.title,
    iconURL:
      'https://upload.wikimedia.org/wikipedia/commons/2/2e/Wikipedia_W_favicon_on_white_background.png',
  });
...
const plugin: InteractionPlugin & AutoCompletePlugin = {
  ...
  async onAutoComplete(interaction: AutocompleteInteraction) {
    ...
    const autocomplete = searchResult.results
      .slice(0, 20)
      .map(x => ({name: x.title, value: `${x.pageid}`}));
