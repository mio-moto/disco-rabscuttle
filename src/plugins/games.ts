...
import { ActionRow, ButtonBuilder, ButtonComponent, ButtonInteraction, ButtonStyle, ChatInputCommandInteraction, Client, Component, Message } from "discord.js";
import { ButtonPlugin, InteractionPlugin } from "../message/hooks";
...
type RevealStepCustom = Duration & {
    ...
    execute: (message: Message<boolean>) => Promise<any>
};
...
const RevealMessage = (message: string, duration: number = 750): RevealStepMessage => ({type: RevealType.Message, message: message, duration: duration});
const RevealPause = (durationInMilliseconds: number = 750): RevealStepPause => ({type: RevealType.Pause, duration: durationInMilliseconds });
const RevealAction = (action: (message: Message) => Promise<any>, duration: number = 750): RevealStepCustom => ({type: RevealType.Custom, execute: action, duration});
...
const intros: RevealStory[][] = [
    ...
    [
        RevealMessage("My favourite - customer."),
        RevealMessage("Yes, customer.")
    ],
    [
    ...
    [
        RevealMessage("Couldn't have chose any better."),
        RevealMessage("Seriously...")
    ],
    [
        RevealMessage("uwu picked a gweat choice of cawds thewe, mistew~"),
        RevealMessage("Ahem.")
    ],
    [
...
const revealChoreosBusting: RevealStep[][] = [
    ...
    [
        RevealMessage("C'mon man."),
        RevealMessage("I'm already sorry for you."),
        RevealOtherCard,
        RevealOtherCard,
        RevealOtherCard
    ],
    [
        {...RevealClickedCard, duration: 0},
    ...
    [
        RevealOtherCard,
        RevealMessage("||Un||Lucky you!", 1_200),
        {...RevealOtherCard, duration: 0},
        RevealOtherCard,
        RevealMessage("Unlucky you.")
    ],
    [
        RevealOtherCard,
        RevealMessage("Right or left, right or left?"),
        RevealOtherCard,
        RevealClickedCard
    ]
]
...
const revealChoreosWinning: RevealStep[][] = [
    [
        RevealClickedCard,
        RevealMessage("Very good, I'm proud of you.")
    ],
    [
        RevealOtherCard,
        RevealOtherCard,
        RevealMessage("What do you guess is behind the last card?"),
        RevealClickedCard
    ],
    [
        RevealClickedCard,
        RevealMessage("That was quick.")
    ],
    [
        RevealOtherCard,
        RevealMessage("Let's see, what do we have here?"),
        RevealClickedCard,
        RevealMessage("If you just had that luck with your face."),
    ],
    [
        ...
        RevealOtherCard,
        RevealMessage("The 20 meter pacer test will begin in 30 seconds. Line up at the start. The running speed starts slowly, but gets faster each minute after you hear this signal.", 2_500),
...
const revealChoreosRandom: RevealStep[][] = [
    ...
    [
        RevealAction(async (message) => {
            const row = new ActionRowBuilder<ButtonBuilder>()
                .addComponents(
                    buttonBuilder("monte-1", "🍝", ButtonStyle.Secondary, false)
                );
            await message.edit({content: "Oops.", components: [row]})
        })
    ],
    [
        RevealMessage("Uh, hum."),
        RevealOtherCard,
        RevealOtherCard,
        RevealOtherCard
    ],
    [
        ...
        RevealAction(async (message) => {
            const row = new ActionRowBuilder<ButtonBuilder>()
                .addComponents(...buildButtonRow([CardLoss, CardLoss, CardLoss], -1, false))
...
const runStory = async (message: Message<boolean>, choreo: ((RevealStepMessage | RevealStepPause | RevealStepCustom))[]) => await revealCards(message, 0, -1, choreo);
...
const doClickChoreography = async (interaction: ButtonInteraction) => {
    ...
    const selection = parseInt(interaction.customId.split("-")[1]) as Choice;
